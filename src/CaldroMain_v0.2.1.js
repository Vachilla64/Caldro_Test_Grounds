"CALDRO VERSION 0.2.1 {Library Wide update and Partial Physics update}" 
 "FLAGS: "
   "Backwards Compatable Update [BCU]"
   "Stable update"
""

"Main updates: >>> "
  "[NF] (non-final) >>"
    "Physics: Intorduced the 'colliderResolutionEngine2D' class for working with Colliders"
    "Physics: Intorduced seeral Collider Classes"
      "'boxCollider2D' [NF]"
      "'circleCollider2D' [NU]"
      "'polygonCollider2D' [NU]"
  "[SID] (still in development) >>"
    "Vectors: Introduced the 'Lvector2D' (lightwieght Vector) class"
    "Vectors: Introduced the 'vecMath' class to hold vector math operations"
    "Vectors: updated the 'Vector2D' class, it is now an official version"
    "Matrices: remaned the 'matrix' class to 'matrix2D'"
    "Matrices: updated the 'matrix2D' class"
    "Matrices: introced a matrix / array2D utility object 'matUtils'"
    "Arrays; inroduced  an Array utility utility object 'arrUtils'"
""

"sub updates: >>"
  "Bug fixes: Fixed a major bug in the 'keyStateHandler' class present in the v0.2.0 update"
  "Bug fixes: Fixed a minor bug in the 'DOMaudioManeger' class present in the v0.2.0 update"
  "Bug fixes: fixed a bug that stopped text from giving off the 'glow' effect"
  "Caldro: added the 'safetyDeltatimeCap' property to the Caldro.time object"
  "Caldro: added the 'getAverageFrameRate' function to the Caldro.time object"
  "Matrices: added an 'array2D' matrix creator function"
""

"Notes >=>"
  "Caldro is currently under a standardization stage (library wise), the target standard version number is 1.0.0"
  "The 'Caldro Testing Enviroment' [CTE]"
    "The CTE is a brute-force testing enviroment currently [SID] created to monitor and compare JavaScript operations"
    "This lead to the creation of the Lightweight Vector Class 'Lvector2D' and the 'vecMath' object"
    "This enviroment is planned to be published with Caldro v1.0.0 or earlier"
...
class colliderResolutionEngine2D {
	constructor() {
		...
		this.colliderRenderingLineWidth = 4
	}
	addBody(body) {
	...
	renderColliders() {
		for (let body of this.bodies) {
			if (getConstructorName(body) == "boxCollider2D") {
				...
				if(body.static){
					stRect(body.x, body.y, body.width, body.height, "white", this.colliderRenderingLineWidth)
				}
				...
				if(this.renderingStyle.includes("stroke")){
					stRect(body.x, body.y, body.width, body.height, color, 1, color, this.colliderRenderingLineWidth)
...
class matrix2D {
    ...
    getRow(rowIndex) {
        return this.mat[rowIndex]
    }
    getColumn(columnIndex) {
        let arr = new Array();
        for (let row = 0; row < this.mat.length; ++row) {
            arr.push(this.mat[row][columnIndex])
        }
        return arr;
    }
}
...
// rendering classes
class imageHandler {
	constructor(drawingCanvas = document.createElement("canvas")) {
	...
	addImage(imageID, src, ImageWidth = 64, ImageHeight = 64) {
	...
	draw(imageID, x = 0, y = 0, width = null, height = null, centralized = false, angle = 0) {
		...
		if (centralized) {
			...
			context.translate(x, y);
			context.rotate(degToRad(angle));
			// context.drawImage(img, drawing.x, drawing.y, drawing.width, drawing.height, -width / 2, -height / 2, width, height);
...
function fillColor(color = "skyblue", context = Caldro.rendering.context) {
	if (context.fillStyle != color) {
		if (typeof color == "string") {
			...
			context.fillStyle = colorObjectToString(color)
		}
	}
}
...
function drawRegularPolygon(x = 0, y = 0, radius = 1, numberOfVertices = 3, color = "skyblue") {
...
function wrapText(text, x, y, maxWidth, lineHeight, color = "green", font = "50px Arial", angle = 0) {
	Caldro.rendering.context.save()
...
class particle {
	...
	render() {
		...
		if (typeof this.renderingFunction == 'function') {
			...
			line(this.x, this.y, this.x + this.xv * this.size, this.y + this.yv * this.size, this.color, 6);
		} else {
...
var touchSwipeStartPoint = new Point2D(0, 0);
