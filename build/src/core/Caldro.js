...
const setDefaultParam = (key, value) => {
    if (!Caldro_DefaultParam.hasOwnProperty(key))
        return;
    // @ts-ignore
    Caldro_DefaultParam[key] = value;
};
// @ts-ignore
const getDefaultParam = (key) => Caldro_DefaultParam[key];
const init = (initParam) => {
    ...
    if (!ctx)
        throw new GLCreationError(Caldro_DefaultParam.renderer);
...
export { init, getError, start, getDefaultParam, requestFullScreen };
